...
class MapManager {
    ...
    initMap(containerId) {
        ...
        this.map = L.map(containerId).setView([14.5, -14.5], 8);
    ...
    addCommuneMarker(commune, stats) {
        if (!this.map || !this.markerCluster) return;

        const latlng = this.getLatLngForCommune(commune);
        if (!latlng) return;

        const marker = L.marker(latlng).bindPopup(`
            <b>${commune}</b><br>
            Total parcelles: ${stats.total.toLocaleString()}<br>
            NICAD: ${stats.nicad_oui.toLocaleString()}<br>
            Délibérées: ${stats.deliberees_oui.toLocaleString()}<br>
            Superficie: ${stats.superficie.toLocaleString(undefined, { maximumFractionDigits: 2 })} ha
        `);
    ...
    getLatLngForCommune(commune) {
        const communeCoords = {
            'NDOGA BABACAR': [14.7, -14.3],
            'BANDAFASSI': [12.5, -12.3],
            'DIMBOLI': [13.2, -13.8],
            'MISSIRAH': [14.0, -14.5],
            'NETTEBOULOU': [13.5, -12.7]
        };
        return communeCoords[commune] || null;
    }
...
class ChartManager {
    ...
    createLine(canvasId, config) {
        this.createChart(canvasId, 'line', {
            ...config,
            options: {
                responsive: true,
                plugins: {
    ...
    createDoughnut(canvasId, config) {
        this.createChart(canvasId, 'doughnut', {
            ...config,
            options: {
                responsive: true,
                plugins: {
    ...
    createPolar(canvasId, config, options) {
        this.createChart(canvasId, 'polarArea', {
            ...config,
            options: {
                responsive: true,
                plugins: {
    ...
    createTemporalChart(canvasId, data) {
        this.createChart(canvasId, 'line', {
    ...
    createTopoTypeDonutChart(canvasId, data) {
        this.createDoughnut(canvasId, {
            labels: ['Champs', 'Bâtis'],
            datasets: [{
                data: [data.champs, data.batis],
                backgroundColor: [this.colors.success, this.colors.secondary],
                borderWidth: 2,
                borderColor: '#ffffff'
            }]
    ...
    createEtatCommuneBarChart(canvasId, communes, etats) {
        const etatCounts = communes.reduce((acc, commune, i) => {
            const etat = etats[i] || 'Non défini';
            if (!acc[commune]) acc[commune] = {};
            acc[commune][etat] = (acc[commune][etat] || 0) + 1;
            return acc;
        }, {});

        const labels = Object.keys(etatCounts);
        const uniqueEtats = [...new Set(etats.filter(e => e))];
        const datasets = uniqueEtats.map((etat, i) => ({
            label: etat,
            data: labels.map(commune => etatCounts[commune][etat] || 0),
            backgroundColor: this.colors.chartColors[i % this.colors.chartColors.length]
        }));

        this.createBar(canvasId, {
            labels,
            datasets
        });
    }
